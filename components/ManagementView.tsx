...
const ScrollToTopButton = ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => {
    ...
    return (
        <button onClick={() => containerRef.current?.scrollTo({ top: 0, behavior: 'smooth' })} className="absolute bottom-4 right-4 bg-slate-900/50 dark:bg-white/20 hover:bg-emerald-500 text-white p-2 rounded-full shadow-lg backdrop-blur-sm transition-all z-20 animate-in fade-in zoom-in" title="Voltar ao Topo"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 10l7-7m0 0l7 7m-7-7v18" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
    );
};
...
const ManagementView: React.FC<ManagementViewProps> = ({ 
  ...
  const [selectedSector, setSelectedSector] = useState<ProductionStep>(
      currentUser.departamento !== 'Geral' ? (currentUser.departamento as ProductionStep) : 'preImpressao'
  );
  
  // Filtros e Estados de Visualização (Igual ao Kanban)
  ...
  const [expandedDetails, setExpandedDetails] = useState<Record<string, boolean>>({}); // Para detalhes inline
  ...
  const expandAllGroups = () => {
      const allOrs = sectorOrders.map(o => o.or);
      const newState = allOrs.reduce((acc, or) => ({ ...acc, [or]: true }), {});
  ...
  const collapseAllCards = () => {
      const allIds = sectorOrders.map(o => o.id);
  ...
  // 1. Filtrar Usuários do Setor (Colunas) - ADMIN INCLUÍDO
  const teamMembers = useMemo(() => {
      return users.filter(u => 
          u.id !== currentUser.id && 
          (
              u.departamento === selectedSector || 
              u.departamento === 'Geral' || 
              u.role === 'Admin' // Admin aparece em todos os setores
          )
      );
  }, [users, selectedSector, currentUser]);

  // 2. Filtrar Ordens do Setor
  const sectorOrders = useMemo(() => {
      return orders.filter(o => {
          const validStatus = !o.isArchived && 
              (o[selectedSector] !== 'Concluído' 
              || (o[selectedSector] === 'Pendente' && !o.isArchived)); 
          
          if (!validStatus) return false;
          if (priorityFilter !== 'ALL' && o.prioridade !== priorityFilter) return false;
          if (searchTerm) {
              const term = searchTerm.toLowerCase();
              return o.cliente.toLowerCase().includes(term) || 
  ...
  }, [orders, selectedSector, priorityFilter, searchTerm]);

  // 3. Distribuir Ordens nas Colunas (Users)
  const kanbanData = useMemo(() => {
      ...
      sectorOrders.forEach(o => {
          const assignment = o.assignments?.[selectedSector];
          // Se atribuído e usuário existe na view atual, vai pra ele. Senão 'unassigned'.
          const targetKey = assignment && assignment.userId && data[assignment.userId] ? assignment.userId : 'unassigned';
          
          if (!tempGrouping[targetKey][o.or]) tempGrouping[targetKey][o.or] = [];
          tempGrouping[targetKey][o.or].push(o);
      });

      [...teamMembers.map(u => u.id), 'unassigned'].forEach(key => {
          ...
          // Sorting Groups: Active > High Prio > Date
          groups.sort((a, b) => {
              const itemA = a.items[0];
              const itemB = b.items[0];
              
              const statusA = a.items.some(i => i[selectedSector] === 'Em Produção');
              const statusB = b.items.some(i => i[selectedSector] === 'Em Produção');
      ...
      return data;
  }, [sectorOrders, teamMembers, selectedSector]);
  ...
  // --- CARD COMPONENT ---
  const OrderCard = ({ order, isAssigned }: { order: Order, isAssigned: boolean }) => {
      const status = order[selectedSector];
      const assignment = order.assignments?.[selectedSector];
      const isInProgress = status === 'Em Produção';
      const isLate = !order.isArchived && order.dataEntrega < new Date().toLocaleDateString('en-CA');
              ...
              <div className="flex flex-col p-2.5 flex-1 relative">
                  ...
                  <div className="flex justify-between items-center mt-auto pt-2 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); onAssignUser(order.id, selectedSector); }} className="h-6 px-3 rounded-lg flex items-center gap-1 justify-center transition-all shadow-sm active:scale-95 shrink-0 bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white border-blue-200 dark:bg-blue-900/20 dark:text-blue-400" title={isAssigned ? "Gerenciar Atribuição" : "Atribuir Tarefa"}>
                              {isAssigned ? (<svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" strokeWidth="2"/><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" strokeWidth="2"/></svg>) : (<span className="text-[8px] font-black uppercase">ATRIBUIR</span>)}
                          </button>
                      </div>
                  </div>
              </div>
  ...
  // --- COLUMN LAYOUT ---
  const TeamColumn = ({ id, label, avatar, count, isActive, groups }: { id: string; label: string; avatar: React.ReactNode; count: number; isActive: boolean; groups: { or: string, client: string, items: Order[] }[] }) => {
          ...
          <div className={`w-[340px] flex-shrink-0 flex flex-col h-full md:rounded-t-2xl border-x border-t border-b-0 transition-all duration-300 relative ${isActive ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800' : 'bg-slate-100/50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-800'}`}>
              ...
              <div ref={columnRef} className="flex-1 overflow-y-auto custom-scrollbar p-2 flex flex-col gap-2 pb-4 relative">
                  {groups.map(group => {
                      ...
                      const activeItems = group.items.filter(i => i[selectedSector] === 'Em Produção');
                      const isGroupActive = activeItems.length > 0;
                      const isLate = group.items.some(i => !i.isArchived && i.dataEntrega < new Date().toLocaleDateString('en-CA'));
                      ...
                      if (!isMulti) return <OrderCard key={firstItem.id} order={firstItem} isAssigned={id !== 'unassigned'} />;
                      ...
                      return (
                          <div key={group.or} className="mb-2">
                              ...
                              {isExpanded && (<div className="mt-2 pl-3 ml-2 border-l-2 border-slate-200 dark:border-slate-700 space-y-2 animate-in slide-in-from-left-2">{group.items.map(item => <OrderCard key={item.id} order={item} isAssigned={id !== 'unassigned'} />)}</div>)}
                          </div>
                      );
                  })}
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900/50 relative overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-3 flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto flex-1 justify-end">
              {isAdmin && (<div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl shadow-sm flex gap-1 overflow-x-auto custom-scrollbar">{Object.keys(DEPARTMENTS).map((dept) => (<button key={dept} onClick={() => setSelectedSector(dept as ProductionStep)} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap ${selectedSector === dept ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>{DEPARTMENTS[dept as ProductionStep].split(' ')[0]}</button>))}</div>)}
              {!isAdmin && (<div className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-200 dark:border-emerald-800">Setor: {DEPARTMENTS[selectedSector]}</div>)}
          </div>
      </div>

      {/* Toolbar */}
      <div className="px-4 py-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 shrink-0 z-10">
          <div className="flex items-center gap-4 overflow-x-auto custom-scrollbar flex-1">
              <div className="relative min-w-[200px] md:w-64"><input type="text" placeholder="Buscar Ref, O.R, Cliente..." className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs font-bold uppercase outline-none focus:ring-2 ring-emerald-500 transition-all dark:text-white" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /><svg className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" strokeWidth="2.5"/></svg></div>
              <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-2 hidden sm:block"></div>
              <div className="flex items-center gap-2"><span className="text-[9px] font-bold text-slate-400 uppercase hidden sm:block">Prioridade:</span>{(['ALL', 'Alta', 'Média', 'Baixa'] as const).map(p => (<button key={p} onClick={() => setPriorityFilter(p)} className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase border transition-all whitespace-nowrap ${priorityFilter === p ? 'bg-slate-800 text-white border-slate-800 dark:bg-white dark:text-slate-900' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700 hover:border-slate-400'}`}>{p === 'ALL' ? 'Todas' : p}</button>))}</div>
              <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-2 hidden sm:block"></div>
              <button onClick={expandAllCards} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Exp. Cards</button>
              <button onClick={collapseAllCards} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Rec. Cards</button>
              <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-2 hidden sm:block"></div>
              <button onClick={expandAllGroups} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Exp. Grupos</button>
              <button onClick={collapseAllGroups} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Rec. Grupos</button>
              <button onClick={expandAllColumns} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Exp. Colunas</button>
              <button onClick={collapseAllColumns} className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-emerald-500 transition-colors text-[9px] font-black uppercase whitespace-nowrap">Rec. Colunas</button>
          </div>
      ...
      <div ref={boardRef} className="flex-1 flex overflow-x-auto overflow-y-hidden px-0 pt-0 md:px-4 md:pt-4 pb-0 gap-3 md:gap-4 custom-scrollbar items-stretch bg-slate-50/50 dark:bg-slate-900/50 scroll-smooth">
          <TeamColumn 
              ...
              count={kanbanData['unassigned'].reduce((acc, g) => acc + g.items.length, 0)}
              isActive={false}
              groups={kanbanData['unassigned']}
          />
          {teamMembers.map(user => {
              const userGroups = kanbanData[user.id];
              const totalItems = userGroups.reduce((acc, g) => acc + g.items.length, 0);
              const inProgressCount = userGroups.reduce((acc, g) => acc + g.items.filter(i => i[selectedSector] === 'Em Produção').length, 0);
              const isBusy = inProgressCount > 0;
              return (<TeamColumn key={user.id} id={user.id} label={user.nome} avatar={<div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black uppercase text-white shadow-sm ${isBusy ? 'bg-amber-500 ring-2 ring-amber-300 dark:ring-amber-800 animate-pulse' : 'bg-emerald-500'}`}>{user.nome[0]}</div>} count={totalItems} isActive={isBusy} groups={userGroups} />);
          })}
      </div>
    </div>
  );
